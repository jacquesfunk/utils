...
dbx_not_in_rs = sorted({dbx for dbx in dbx_set if not any(is_match(dbx, rs) for rs in rs_set)})

# Find fields in rs but not in dbx
rs_not_in_dbx = sorted({rs for rs in rs_set if not any(is_match(rs, dbx) for dbx in dbx_set)})
