...
def count_exit_in_column(df):
    exit_counts = {
        column: df.select(
            pl.col(column).cast(pl.Utf8).str.to_lowercase().str.count_matches('exit', literal=True)
        ).sum().item()
...
def calculate_differences(counts1, counts2, columns):
    return {column: counts2.get(column, 0) - counts1.get(column, 0) for column in columns}
...
sorted_columns_diff = sorted(df1.columns, key=lambda column: differences[column], reverse=False)

# Print results
print_results(total_rows_df1, sorted_df1, count_df1, "Sorted column order by number of 'exit' occurrences in old dataset")
print_results(total_rows_df2, sorted_df2, count_df2, "Sorted column order by number of 'exit' occurrences in new dataset")
