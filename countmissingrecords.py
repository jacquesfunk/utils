...
df1_not_in_df2 = df1.filter(~df1['factorsoft_client_number__c'].is_in(df2['masterclientno'])).unique()
df1_not_in_df2_count = df1_not_in_df2.height

# Count of records that are in df2 but not df1. Drop all duplicates.
df2_not_in_df1 = df2.filter(~df2['masterclientno'].is_in(df1['factorsoft_client_number__c'])).unique()
