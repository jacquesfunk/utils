...
# Output file
output_file = Path(fr"C:\Users\mahmad\OneDrive - Ryan RTS\Downloads\salesforce_responses_{current_date}.csv")
                ...
                if isinstance(json_data, list):
                    for item in json_data:
                        if isinstance(item.get('SalesforceResponse'), dict):
                            ...
                            item['SalesforceResponse']['Account'] = item.get('Account__c', '')

                            # Extract statusCode from SalesforceResponse errors
                            errors = item['SalesforceResponse'].get('errors', [])
                            if errors and isinstance(errors, list) and 'statusCode' in errors[0]:
                                item['SalesforceResponse']['statusCode'] = errors[0]['statusCode']
                            else:
                    ...
                    json_data['SalesforceResponse']['Account'] = json_data.get('Account__c', '')

                    # Extract statusCode from SalesforceResponse errors
                    errors = json_data['SalesforceResponse'].get('errors', [])
                    if errors and isinstance(errors, list) and 'statusCode' in errors[0]:
                        json_data['SalesforceResponse']['statusCode'] = errors[0]['statusCode']
                    else:
