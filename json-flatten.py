...
df = df.apply(lambda row: extract_nested_arrays(row, 'parameters_transittimevariance'), axis=1)
df = df.apply(lambda row: extract_nested_arrays(row, 'parameters_distancetiersdistancetiers'), axis=1)
df = df.apply(lambda row: extract_nested_arrays(row, 'parameters_leadtimetimes'), axis=1)
df = df.apply(lambda row: extract_nested_arrays(row, 'parameters_rateenginesweightengines'), axis=1)
df = df.apply(lambda row: extract_nested_arrays(row, 'parameters_crossborderlanes'), axis=1)
df = df.apply(lambda row: extract_nested_arrays(row, 'parameters_internationalshipmentlanes'), axis=1)
df = df.apply(lambda row: extract_nested_arrays(row, 'parameters_extrachargesandrestrictionsextrachargesandrestrictions'), axis=1)
df = df.apply(lambda row: extract_nested_arrays(row, 'additionaldata_emailnotificationemails'), axis=1)
df = df.apply(lambda row: extract_nested_arrays(row, 'additional_data.equipmenttypekeywords0'), axis=1)
df = df.apply(lambda row: extract_nested_arrays(row, 'additional_data.equipmenttypekeywords1'), axis=1)
df = df.apply(lambda row: extract_nested_arrays(row, 'additional_data.equipmenttypekeywords2'), axis=1)
df = df.apply(lambda row: extract_nested_arrays(row, 'additional_data.equipmenttypekeywords3'), axis=1)
df = df.apply(lambda row: extract_nested_arrays(row, 'additional_data.equipmenttypekeywords.4'), axis=1)
...
output_file = r'C:\Users\mahmad\OneDrive - Ryan RTS\Downloads\dbx_parameters_extracted.csv'
