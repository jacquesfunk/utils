...
dforig = dforig.with_columns((pl.col("int_field").cumcount().over(['int_field', 'ext_field']) + 1).alias('number'))

# Add the 'new_or_update' column based on process ID values using map_dict
df = dforig.with_columns(pl.col("processid").map_dict(processid_mapping, default=default_value).alias('new_or_update'))
