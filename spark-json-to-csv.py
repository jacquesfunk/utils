...
def process_json_files(directory):
    ...
    for i, input_file in enumerate(input_files, start=1):
        ...
        df_filtered = df.filter(~col("salesforce_response_metrics").isNotNull() & ~col("input_file").isNotNull())

        # Flatten SalesforceResponse fields, if it exists
        if "SalesforceResponse" in df.columns:
            salesforce_cols = [col(f"SalesforceResponse.{key}").alias(f"SalesforceResponse_{key}")
                               for key in df.select("SalesforceResponse.*").columns]
            df_filtered = df_filtered.drop("SalesforceResponse").select("*", *salesforce_cols)
        ...
        if "SalesforceResponse_success" in df_csv.columns:
            ...
            if results_df.count() > 0:
                results_df.coalesce(1).write.csv(error_csv_file, header=True, mode="overwrite")
