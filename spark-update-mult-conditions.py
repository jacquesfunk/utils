...
df = (
    ...
    .load("C:\\Users\\mahmad\\OneDrive - Ryan RTS\\projects\\rs-migration-classify.xlsx")
)
...
df = df.withColumn("Business_Unit", lit("N/A")) \
...
for condition, values in conditions:
    df = df.withColumn(
        "Business_Unit", when(condition, lit(values["Business_Unit"])).otherwise(col("Business_Unit"))
    ).withColumn(
        "Data_Source", when(condition, lit(values["Data_Source"])).otherwise(col("Data_Source"))
    ).withColumn(
        "Derived_or_Direct", when(condition, lit(values["Derived_or_Direct"])).otherwise(col("Derived_or_Direct"))
    )
...
df = df.withColumn(
    "Business_Unit", when(col("Business_Unit") == "N/A", lit(None)).otherwise(col("Business_Unit"))
).withColumn(
    "Data_Source", when(col("Data_Source") == "N/A", lit(None)).otherwise(col("Data_Source"))
).withColumn(
    "Derived_or_Direct", when(col("Derived_or_Direct") == "N/A", lit(None)).otherwise(col("Derived_or_Direct"))
)

# Write the updated DataFrame to an Excel file
df.write.format("com.crealytics.spark.excel") \
    .option("header", "true") \
    .save("C:\\Users\\mahmad\\OneDrive - Ryan RTS\\projects\\rs-migration-classify-test.xlsx")

# Define columns to save for updates only
columns = [
    "Report ID", "workspace_id", "pbi_workspace_name", "dataset_name",
    "db_type", "db_name", "schema_name", "table_name",
    "Business_Unit", "Data_Source", "Derived_or_Direct"
]
