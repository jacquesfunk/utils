...
df1_orig = pl.read_excel(
    r"C:\Users\mahmad\OneDrive - Ryan RTS\Downloads\Tied at Both Levels (1).xlsx", sheet_name="Export"
)
...
df2_orig = df2_orig.with_columns([
    pl.col("cadmoddate").str.extract(r"(\d{4}-\d{2}-\d{2})").str.strptime(pl.Date, "%Y-%m-%d").alias("cadmoddate"),
    pl.col("sfmoddate").str.extract(r"(\d{4}-\d{2}-\d{2})").str.strptime(pl.Date, "%Y-%m-%d").alias("sfmoddate")
])
...
filtered_result.write_csv(fr'C:\Users\mahmad\OneDrive - Ryan RTS\Downloads\{current}_{name}.csv')